...
*       use of posix semaphore .A client process gets a filename from tha user
...
main()
{
	...
	pfile = (char*)mmap(NULL,64,PROT_READ|PROT_WRITE,MAP_PRIVATE,fd1,0);	
	ptr1=ptr+1;
	close(fd1);
	while(i<64)
	{
		*(pfile+i) = 'd';
        ...
        printf("Ptr1(t1):%p \n",ptr1);
	close(fd);
        sem_init(&ptr->sem,0,1);/*Semaphore for child */
        sem_init(&ptr1->sem,0,0);/*Semaphor for parent */
