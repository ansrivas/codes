		      ...
		      )
{
 /* Implemente this function which update each array element */
  ...
  len_first = strlen(len_first);
  len_second = strlen(len_second);

  printf("In Update before Allocation");
  ret = allocate_person(&person, len_first, len_second);
  printf("In Update After Allocation");

  ret = strncpy(person->first_name, first_arg, len_first);
  if (NULL == ret)
  {
    printf("Failure in copy One");
    return FAILURE;

  }
  strncpy(person->second_name, second_arg, len_second);
  if (NULL == ret)
  {
    printf("Failure in copy Second");
    return FAILURE;
  }
  person->first_name[len_first] = '\0';
  person->second_name[len_second]= '\0';
                    ...
                    )
{
 /* Implement this function to deallocate the memory */
  ...
  for (index_1 = 0; index_1 < arg_total; index_1++)
  {
     free(arg_person[index_1]->first_name);
     free(arg_person[index_1]->second_name);
     free(arg_person[index_1]);
